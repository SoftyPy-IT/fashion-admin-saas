...
const BillersDrawer = ({
  ...
  const [createBiller, { isSuccess, isLoading }] = useCreateBillerMutation();
  const [
    updateBiller,
  ...
  const onSubmit = async (data: FieldValues) => {
    ...
    try {
      ...
      if (needToUpdate && initialValues) {
        res = await updateBiller({
          id: initialValues.key,
          data,
        }).unwrap();
      } else {
        res = await createBiller(data).unwrap();
      }
  ...
  return (
    <Drawer
      title={needToUpdate ? "Update Biller" : "Create Biller"}
      placement="right"
      size="large"
      onClose={onClose}
      open={open}
    >
      ...
      >
        ...
        >
          {needToUpdate ? "Update Biller" : "Create Biller"}
        </Button>
      </AppForm>
    </Drawer>
  );
