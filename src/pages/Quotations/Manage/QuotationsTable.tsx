...
} from "../../../redux/features/quotaions/quotaions.api";
...
export interface TQuotationData {
  date: Date;
  reference: string;
...
const QuotationsTable = () => {
  ...
  const { data, isLoading, isFetching, refetch } = useGetAllQuotationsQuery([
    ...params,
  ]);
  const [handleDelete, { isLoading: isDeleting }] =
    useDeleteQuotationMutation();

  const tableData = data?.data?.map((quotation: any) => ({
    ...quotation,
    key: quotation._id,
  }));
  ...
  const columns: ProColumns<TQuotationData>[] = [
    {
      title: "Date",
      dataIndex: "date",
      key: "date",
    ...
    },
    {
      ...
      render: (_dom: ReactNode, item: any) => (
        <Space>
          <Link to={`/dashboard/quotations/manage/${item._id}`}>
          ...
          <Button type="dashed" size="small" icon={<FaRegFilePdf />}>
            <a
              href={`${baseURL}/quotations/pdf/${item._id}`}
              target="_blank"
              rel="noreferrer"
            >
              PDF
